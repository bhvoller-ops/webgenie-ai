-- Part 1/7 of the WebGenie production migration reconciliation audit.
-- READ-ONLY. See scripts/db-audit/prod-migration-audit-017-036.sql for the
-- full combined reference version and its header comment (identical
-- content, split into one file per result set because `supabase db query`
-- only surfaces the LAST statement's result set per invocation).
